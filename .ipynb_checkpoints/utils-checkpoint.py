...
# # Define paths
# base_path = '/home/user/persistent/chest_xray'
# out_dir = '/home/user/persistent/chest_xray/data/CSVs'
# splits = ['train', 'val']  
...
# # Function to generate CSVs for each split
# for split in splits:
#     data = []
...
#             name = os.path.splitext(image_name)[0]  # Extract name without extension
#             data.append({'Path': image_path, 'Label': label, 'Name': name})
    
#     # Save CSV for the split
#     df = pd.DataFrame(data)
#     csv_path = os.path.join(out_dir, f'{split}.csv')
#     df.to_csv(csv_path, index=False)
#     print(f"CSV for {split} split saved at: {csv_path}")
